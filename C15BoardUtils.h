...
Board * createBoard(int rows, int cols){
	...
	int **tab = (int **)calloc(rows,sizeof(int *));
	int i;
...
int checkBoard(Board * board){
	int order = 1, rows = board->rows, cols = board->cols;
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
		{
			if (i == rows - 1 && j == cols - 1 && board->table[rows - 1][cols - 1] == 0)
				return 1;
			else if (board->table[i][j] != order)
				break;
			order++;
		}
	}
