...
*/
int isValidInput(char letter){
	if (letter == 'w' || letter == 'a' || letter == 's' || letter == 'W' || letter == 'd' || letter=='A'||letter=='S'||letter=='D')
	return 1;
	return 0;
}
/*
...
void playMove(Board *board, User *user, char choice){
	...
	int step1op=processInput(board, choice);
	modifyMoveCountBasedOnProccessInput(user, step1op);
